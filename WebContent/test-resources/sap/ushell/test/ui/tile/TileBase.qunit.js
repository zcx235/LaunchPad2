...
(function () {
    ...
    asyncTest("Render Tile - URL Target Test", function () {
        baseTile.placeAt('testContainer');
        setTimeout(function () {
            start();
            var bLinkWrapperClassAdded = testContainer.find('.sapUshellTileBaseWrapperLink').length > 0,
                sHrefTargetUrl = testContainer.find('a:first').attr('href'),
                tabIndex = testContainer.find('a:first').attr('tabindex');

            //Test tile rendering when target url is defined with semantic object
            ok(bLinkWrapperClassAdded, "CSS Class: 'sapUshellTileBaseWrapperLink' is added when targetUrl is defined");
            ok(tabIndex === '2', "tab index test");
            ok(sHrefTargetUrl === demiTileData.targetURL, "Url Target matches href Test");
        }, 0);
    });


    asyncTest("Render Tile - Open in new window Test", function () {
        baseTile.setTargetURL('hashLessUrlTarget');
        baseTile.placeAt('testContainer');
        setTimeout(function () {
            start();
            var bIsTargetBlank = testContainer.find('a:first').attr('target') === '_blank';

        //Test tile rendering when target url is not a semantic object (Target doesn't start with: '#').
        ok(bIsTargetBlank, "Target url isn't a Sematic Object Test")
        }, 0);
    });

    asyncTest("Render Tile - No URL Target Test", function () {
        baseTile.setTargetURL('');
        baseTile.placeAt('testContainer');
        setTimeout(function () {
            start();
            var bHasHyperlinkTag = testContainer.find('a').length < 1 ? false : true,
                bIsDataSapUiDefined = testContainer.find('span:first').attr('data-sap-ui') ? true : false,
                bIsIdDefined = testContainer.find('span:first').attr('id') ? true : false;

            ok(!bHasHyperlinkTag, "No Hyper Link Tag Test");
            //When No URL Target is define an Id and 'data-sap-ui' attributes should be set by the writeControlData method of the Renderer Manager.
            ok(bIsDataSapUiDefined && bIsIdDefined, "'writeControlData' test");
        }, 0);
    });
    ...
    asyncTest("Render Tile - title Test", function () {
        ...
        setTimeout(function () {
            ...
            var bSapUshellTileBaseTitleClassAdded = testContainer.find('h3:first').attr('class') === 'sapUshellTileBaseTitle',
                ...
                titleTooltip = testContainer.find('h3:first').attr('title'),
                //The expected title text should be a concatenation of the accessibility state prefix with the actual title.
                expectedTitleText = translationBundle.getText("TileTitle_lable") + demiTileData.title;

            ok(bSapUshellTileBaseTitleClassAdded, "CSS Class: 'sapUshellTileBaseTitle' is added on the base-header div");
            ok(ariaLabelTitle === expectedTitleText, "Title added in an aria-label");
            ok(titleTooltip === translationBundle.getText('launchTileTitle_tooltip', [demiTileData.title]), "Tooltip Added test" );
        }, 0);
    });
